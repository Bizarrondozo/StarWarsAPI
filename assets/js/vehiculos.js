...
const pokeData = (data) => {
    ...
    let html = "";
    document.getElementById("datosVehiculos").innerHTML = "";
    data.forEach((pj) => {
        const URL = pj.url;
        return fetch(URL)
            .then((response) => response.json())
...
const llenarDatos = (data, html) => {
    ...
    html += `<p class="card-text">Precio en creditos: ${data.cost_in_credits}</p>`;
    html += '</div>';
    html += '</div>';
    html += '</div>';
    document.getElementById("datosVehiculos").innerHTML += html;
