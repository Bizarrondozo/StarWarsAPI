...
const pokeData = (data) => {
    ...
    let html = "";
    document.getElementById("datosPlanetas").innerHTML = "";
    data.forEach((pj) => {
        const URL = pj.url;
        return fetch(URL)
            .then((response) => response.json())
...
const llenarDatos = (data, html) => {
    ...
    html += `<p class="card-text">Diametro: ${data.diameter}</p>`;
    html += `<p class="card-text">Terreno: ${data.terrain}</p>`;
    html += `<p class="card-text">Clima: ${data.climate}</p>`;
    html += `<p class="card-text">Poblacion: ${data.population}</p>`;
    html += '</div>';
    html += '</div>';
    html += '</div>';
    document.getElementById("datosPlanetas").innerHTML += html;
